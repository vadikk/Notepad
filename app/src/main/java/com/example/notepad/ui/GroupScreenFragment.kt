...
class GroupScreenFragment : Fragment() {
    ...
    private var binding: FragmentGroupScreenBinding? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mainScreenVM.init(TypeScreen.GROUP)
    }
    ...
    override fun onCreateView(
        ...
        binding = FragmentGroupScreenBinding.inflate(inflater, container, false)
        return binding?.root
    }
    ...
    private fun openEditNote(note: Note?) {
        if (note == null || note.password.isEmpty()) {
            val action = GroupScreenFragmentDirections.actionGroupScreenFragmentToCreateNoteFragment(
                note?.uid.toString()
            )
            findNavController().navigate(action)
        } else {
            findNavController().navigate(
                R.id.action_groupScreenFragment_to_passwordFragment,
